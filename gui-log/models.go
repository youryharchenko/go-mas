...
func (m *ManagerBot) Plan(ctx context.Context, msg mas.Envelope) ([]mas.Action, error) {
	...
	if msg.Payload == "DONE" {
		return []mas.Action{
			...
			mas.Send("console", "Worker finished a task. Good job."),
		}, nil
	}

	if msg.Payload == "STOP" {
		return []mas.Action{
			// 2. Змінюємо стан через MutateState
			mas.MutateState(func(a any) {
				a.(*ManagerBot).Active = false
			}),
			mas.Send("console", "I'm stopped"),
		}, nil
	}
	...
	/* if msg.Payload == "TICK" && !m.active {
		return []mas.Action{
			//mas.SayLog("Worker finished a task. Good job."),
			mas.Send("console", "I'm not active"),
		}, nil
	} */
...
func (w *WorkerBot) Plan(ctx context.Context, msg mas.Envelope) ([]mas.Action, error) {
	...
	case WorkOrder:
		return []mas.Action{
			...
			mas.Send("console", fmt.Sprintf("I increased count to %d", w.Count+payload.Amount)),
